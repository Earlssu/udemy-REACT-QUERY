...
test("filter appointments by availability", async () => {
    ...
    // appointments start ouf filtered (show only available)
    const filteredAppointments = getAppointmentCount(result.current.appointments);

    // set to return all appointments
    act(() => result.current.setShowAll(true));

    // wait for count of appointments to be greater than when filtered
    await waitFor(() => expect(getAppointmentCount(result.current.appointments)).toBeGreaterThan(filteredAppointments));
